    ...
    const fvec3& bias)
{
	...
	*edgelistdata = new uint32_t[edgelistsize];
	...
	for (uint32_t n = 0; n < N; n++) {
		for (uint32_t l = 0; l < L - 1; l++) {
			uint32_t Ind0 = l * N * M + n;
			uint32_t Ind1 = l * N * M + (n + 1) % N;
			uint32_t Ind2 = (l + 1) * N * M + n;
			uint32_t Ind3 = (l + 1) * N * M + (n + 1) % N;

			(*facelistdata)[3 * 2 * ((L - 1) * N + l * N + n) + 0 + 0] = Ind0;
			(*facelistdata)[3 * 2 * ((L - 1) * N + l * N + n) + 0 + 1] = Ind2;
			(*facelistdata)[3 * 2 * ((L - 1) * N + l * N + n) + 0 + 2] = Ind1;

			(*facelistdata)[3 * 2 * ((L - 1) * N + l * N + n) + 3 + 0] = Ind2;
			(*facelistdata)[3 * 2 * ((L - 1) * N + l * N + n) + 3 + 1] = Ind3;
			(*facelistdata)[3 * 2 * ((L - 1) * N + l * N + n) + 3 + 2] = Ind1;
		}
	}
	...
	for (uint32_t n = 0; n < N; n++) {
		for (uint32_t m = 0; m < M - 1; m++) {
			uint32_t Ind0 = m * N + n;
			uint32_t Ind1 = m * N + (n + 1) % N;
			uint32_t Ind2 = (m + 1) * N + n;
			uint32_t Ind3 = (m + 1) * N + (n + 1) % N;

			(*facelistdata)[3 * (4 * N * (L - 1) + 2 * (m * N + n)) + 0 + 0] = Ind0;
			(*facelistdata)[3 * (4 * N * (L - 1) + 2 * (m * N + n)) + 0 + 1] = Ind1;
			(*facelistdata)[3 * (4 * N * (L - 1) + 2 * (m * N + n)) + 0 + 2] = Ind2;

			(*facelistdata)[3 * (4 * N * (L - 1) + 2 * (m * N + n)) + 3 + 0] = Ind2;
			(*facelistdata)[3 * (4 * N * (L - 1) + 2 * (m * N + n)) + 3 + 1] = Ind1;
			(*facelistdata)[3 * (4 * N * (L - 1) + 2 * (m * N + n)) + 3 + 2] = Ind3;
		}
	}

	for (uint32_t n = 0; n < N; n++) {
		for (uint32_t m = 0; m < M - 1; m++) {
			uint32_t Ind0 = (L - 1) * M * N + m * N + n;
			uint32_t Ind1 = (L - 1) * M * N + m * N + (n + 1) % N;
			uint32_t Ind2 = (L - 1) * M * N + (m + 1) * N + n;
			uint32_t Ind3 = (L - 1) * M * N + (m + 1) * N + (n + 1) % N;

			(*facelistdata)[3 * (4 * N * (L - 1) + 2 * (N * (M - 1)) + 2 * (m * N + n)) + 0 + 0] = Ind0;
			(*facelistdata)[3 * (4 * N * (L - 1) + 2 * (N * (M - 1)) + 2 * (m * N + n)) + 0 + 1] = Ind2;
			(*facelistdata)[3 * (4 * N * (L - 1) + 2 * (N * (M - 1)) + 2 * (m * N + n)) + 0 + 2] = Ind1;

			(*facelistdata)[3 * (4 * N * (L - 1) + 2 * (N * (M - 1)) + 2 * (m * N + n)) + 3 + 0] = Ind2;
			(*facelistdata)[3 * (4 * N * (L - 1) + 2 * (N * (M - 1)) + 2 * (m * N + n)) + 3 + 1] = Ind3;
			(*facelistdata)[3 * (4 * N * (L - 1) + 2 * (N * (M - 1)) + 2 * (m * N + n)) + 3 + 2] = Ind1;
		}
	}
