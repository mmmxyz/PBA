...
class ClothMesh {
	    ...
	    , tva()
	{
		...
		InnerEdgeCList.reserve((N - 1) * (N - 1) + 2 * (N - 2) * (N - 2));
...
int main(int argc, char const* argv[])
{
	...
	ClothMesh CM0(64, 8.0, fvec3(0.0, 3.0, 0.0));

	Init(CM0.RestPositionList.data(), CM0.TriIndList.data(), CM0.InnerEdgeIndList.data(), CM0.InnerEdgeCList.data(), CM0.AList.data(), CM0.VList.data(), CM0.N);
